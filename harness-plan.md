...
$T codex exec --cd "$(pwd)" > ".cache/codex-reviews/plan-${TASK_ID}-${TS}.json" 2> ".cache/codex-reviews/plan-${TASK_ID}-${TS}.stderr" <<EOF
...
$T codex exec --cd "$(pwd)" > ".cache/codex-reviews/diff-${TASK_ID}-${TS}.json" 2> ".cache/codex-reviews/diff-${TASK_ID}-${TS}.stderr" <<EOF
...
- [ ] **0c. macOS / `gh` / git 환경 확인**
  - [ ] hard timeout provider(`timeout`/`gtimeout`/python wrapper) 확보
  - [ ] §2-2 provider 패턴 검증
