...
> **상태**: Draft v6 — 5차 리뷰 반영 완료 (마감 정리), 구현 착수 가능. Phase 0 검증만 통과하면 실행.
> **작성**: 2026-04-18
...
- **G1b. decision prompt 횟수**: 사용자 게이트 응답 횟수 = **베이스라인 측정 + 의식적 통제** (목표치는 Phase 4 후 확정. v5 §6-2 의 conditional 분류 반영 시 정상 경로 **평균 3~4회** — always 3개 + conditional 중 P0/P1 발생 시 평균 ~1회. 이상 경로일수록 상승)
...
| GP-2 | conditional | `/plan` codex 리뷰에서 P0/P1 ≥ 1건 | **P0/P1 없음 → 자동 통과 (P2 는 audit log 에만 기록, 계획서 자동 수정 X)** | 리뷰 항목 반영 선택 |
| GW-1 | always | `/work` 시작 시점 | — | "브랜치 명 확정?" |
| GW-2 | conditional | `/work` diff 리뷰에서 P0/P1 ≥ 1건 | P0/P1 없음 → 자동 통과 (P2 는 audit log 에만 기록) | 리뷰 항목 반영 선택 |
...
**conditional 게이트 (4개)**: GP-1, GP-2, GW-2, GS-1 — 이상 신호가 있을 때만 개입. 정상 경로에서는 자동 통과해 G1b (decision prompt 수) 축소.

**기대 효과**: 정상 경로 평균 3~4회 개입 (always 3 + conditional 중 P0/P1 발생 시 평균 ~1회). 이상 경로일수록 더 자주 멈춤 — 사용자는 "평상시 조용, 이상 시 집중" 경험.

> **R5 (ㅇㅋ 자동화) 와의 충돌 해소**: conditional 자동 통과는 (a) P0/P1 이 0건일 때만, (b) P2 는 audit log 에 자동 기록되어 회고 시 재확인 가능, (c) 이 정책 자체를 Q26 에서 재리뷰. "일 없을 때 조용히 넘기는 것" 과 "ㅇㅋ 자동화" 는 다름 — 전자는 판단할 게 없는 경우, 후자는 판단해야 할 것을 건너뛰는 경우.
...
5. 계획서 초안 작성 (템플릿은 §10-1 참조). 작업 항목은 stable id (`P1.`, `P2.`, ...) 필수
6. **run_id 결정**: `RUN_ID="plan#$((${loop_count_by_command.plan:-0}+1))"`
7. Codex 리뷰 호출 (§7-1 호출 규약 — 프롬프트에 `${RUN_ID}` 주입). raw JSON 은 `.cache/codex-reviews/plan-<task-id>-<ts>.json` 에 저장
8. 결과 파싱 후 §7-3 스키마 검증. `run_id` 응답값이 주입값과 일치하지 않으면 §7-5-A fallback
9. **조건부 (GP-2)**: 응답 items 에 severity ∈ {P0, P1} 이 **1건 이상** → 사용자 표 제시 + 게이트 실행. **P0/P1 0건 → 자동 통과** (default 정책: P2 는 audit log 에만 기록, 자동 수용 X). audit log 에는 어느 쪽이든 append
10. 선택 항목을 계획서에 반영. state 의 `review_runs[]` 에 `{run_id, command: "plan", loop, ts, raw_path, accepted_ids, rejected_ids, deferred_ids}` append, `loop_count_by_command.plan` 증가
11. 루프 판정: `loop_count_by_command.plan < 3` 이고 사용자가 [4]/[5] 미선택이면 Step 6로 복귀
12. 종료 시 state 파일에 `stage: "plan.done"` 기록

**중단 후 재개**: `stage`, `loop_count_by_command.plan`, `review_runs[]` (plan# prefix 만) 로 재진입 지점 결정
...
3. 계획서 항목 순회하며 구현. 항목 완료 시 state 의 `completed_plan_items[]` 에 해당 **stable id** (예: `"P1"`, `"P2"` — §10-1 규약) 를 append
...
6. **run_id 결정**: `RUN_ID="work#$((${loop_count_by_command.work:-0}+1))"`. Codex diff 리뷰 호출 (§7-2 호출 규약, diff 크기 분기는 §7-4 — 프롬프트에 `${RUN_ID}` 주입)
7. 결과 파싱 후 §7-3 스키마 검증. `run_id` 불일치 시 §7-5-A fallback. 표 제시 — raw JSON 은 `.cache/...` 에 저장
8. **조건부 (GW-2)**: 응답 items 에 P0/P1 ≥ 1건 → 사용자 표 + 게이트. **P0/P1 0건 → 자동 통과** (§6-2). P2 는 어느 쪽이든 audit log 에만 기록, `docs/plans/.audit/<task-id>.md` 에 append
9. 선택 항목 적용. state 의 `review_runs[]` 에 해당 run 엔트리 (`run_id`, `command: "work"`, `loop`, `ts`, `raw_path`, `accepted_ids`, `rejected_ids`, `deferred_ids`) append, `loop_count_by_command.work` 증가 → Step 6로 루프 (`loop_count_by_command.work < 3` 조건)
...
3. **커밋 분할 제안 수행** (분할 기준은 §10-3) → **GS-2 게이트** (always) 분할 미리보기 확인 → 승인 후 → `stage=ship.partition.previewed` 기록
4. **커밋 생성 수행**: 그룹별 `git add <files>` + `git commit` 순차 실행 (`-A` 금지, 파일 명시). 각 커밋 직후 sha 를 state 의 `created_commits[]` 에 append. 중단 후 재진입 시 이 배열과 `git log` 비교로 **재커밋 방지**. 모든 커밋 완료 후 → `stage=ship.commits.created` 기록
...
7. **Push 수행**: `git push -u origin <branch>`
   - 성공 후 → state 의 `push_status: "pushed"`, `remote_branch: "<branch>"`, `stage=ship.pushed` 기록
   - `git push` 가 이미 up-to-date 이면 멱등적으로 다음 단계
   - 실패 시 §7-5-C ladder
8. **PR 생성 수행**: `gh pr create --body-file .cache/pr-body-${TASK_ID}.md ...`
   - 성공 후 → state 의 `pr_url`, `stage=ship.pr.created` 기록
   - 실패 시 §7-5-D ladder (본문 `.cache/pr-body-*.md` 재사용)
9. **`/done` 로직 수행** (TASKS `🔄`→`✅`, progress, ADR 갱신)
...
| 현재 stage | 확인할 것 | 다음 재진입 지점 |
|-----------|----------|------------|
| 없음 / `work.done` | — | Step 1 부터 (precheck 부터 시작) |
| `ship.precheck` (통과) | — | Step 3 (분할 제안 + GS-2) |
| `ship.partition.previewed` | — | Step 4 (커밋 생성) |
| `ship.commits.created` | `created_commits[]` vs `git log` 교차 확인 — 중간 중단 시 남은 커밋만 재생성 | Step 5 (PR 본문 생성) |
| `ship.pushed` | `git ls-remote origin <branch>` 로 원격 반영 확인 | Step 8 (PR 생성). `pr_url==null` 이면 `gh pr create` 재시도 (`.cache/pr-body-*.md` 재사용) |
...
- `review_runs[]`: **각 Codex 호출을 1개 run 으로 기록**. Codex 출력의 `items[].id` 가 호출마다 1,2,3... 으로 reset 되므로 단일 배열로 합치면 loop 간 충돌 발생. run 별로 분리 저장해 감사/회고 시 "어느 호출의 몇 번 항목을 수용/거부했는지" 를 추적
  - `run_id`: `"{command}#{loop}"` 형식의 복합 키 (예: `plan#1`, `plan#2`, `work#1`)
  - `accepted_ids` / `rejected_ids` / `deferred_ids`: 해당 run 내 items[].id
  - `raw_path`: 원본 Codex JSON 위치 (audit 감사 흔적)
- `loop_count_by_command`: 커맨드별 호출 수 (§7-6 상한 3회 판정에 사용)
...
RUN_ID="plan#1"       # v5: 호출부가 loop_count_by_command.plan 기반으로 결정해 주입
...
TS=$(date +%s)
TASK_ID="<task-id>"
RUN_ID="work#1"   # v5: 호출부가 loop_count_by_command.work 기반으로 결정해 주입
...
| 필드 | 필수 | 값 |
|------|------|------|
| `run_id` | ✓ (v4) | `"{command}#{loop}"` (예: `plan#1`, `work#2`). 호출부에서 주입. state 의 `review_runs[].run_id` 와 동일 키 |
| `summary` | ✓ | 1줄 |
...
> **F2 수정 핵심**: `items[].id` 만으로는 호출 간 유일성이 없음. 오케스트레이터는 호출 전 `run_id` 를 결정(`plan#N`, `work#N`)하고, 프롬프트에 "응답 JSON 최상위에 `run_id: \"<값>\"` 를 그대로 포함" 명령을 추가. 복합 키 `{run_id}:{id}` 가 audit 및 state 저장용 정체성.
...
| 4 | `run_id` | string | `plan#1` \| `work#2` | state `review_runs[].run_id` 와 조인 키 |
...
2026-04-19T14:23:00Z	task-3-4	plan	plan#1	1	plan	-1	4821	1207	8234	ok	none			
2026-04-19T16:10:07Z	task-3-4	work	work#1	1	diff	1234	52018	3402	21033	timeout	review_skip			
```
...
- **lock 이 PID 기반 단순 구현** — 강한 보장은 `flock(1)` 필요하나 macOS 기본 미포함. 단일 사용자 전제에서 충분 (디렉토리 원자성 + PID stale 처리는 §6-4-4)
...
| **decision prompt 수** (G1b) | 게이트 응답 카운트 (audit log 줄 수) | 베이스라인 측정 후 결정. v5 §6-2 conditional 반영 시 정상 경로 추정 **3~4**, 이상 경로는 상승 |
