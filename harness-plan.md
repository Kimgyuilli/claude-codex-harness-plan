...
> **상태**: Draft v8 — 서브에이전트 병렬 리뷰 1차 통합 반영 완료. **Phase 0a/0b 통과 전에는 구현 착수 불가**.
> **작성**: 2026-04-18
...
> **핵심 가정의 미검증 상태 (F1 Blocker)**: 본 설계 전반은 "슬래시 커맨드가 다단계 게이트/루프/하위 호출을 지원한다" 는 가정 위에 서 있으며, 이 가정이 거짓이면 §6~§13 대부분이 무효. Phase 0a (§13) 와 Phase 0b (§13) 검증 전까지 본 문서는 **조건부 설계안**이며, 상세 절차/명령 예시는 후보안으로만 취급한다.
...
| 자산 | 위치 | 활용 |
|------|------|------|
| `.claude/commands/sync.md` | PeakCart | 그대로 유지, `/plan` 에서 **로직 참조** (nested slash 는 Phase 0 전까지 가정 금지) |
...
- **G1b. decision prompt 횟수**: 사용자 게이트 응답 횟수 = **베이스라인 측정 + 의식적 통제**
  ...
  - 목표치는 Phase 4 후 확정. 현재 가설은 정상 경로 **평균 3~4회 응답** (always 3개 + conditional 중 P0/P1 발생 시 평균 ~1회), 이상 경로일수록 상승
- **G1c. context switch 횟수**: 사용자가 Claude ↔ Codex 도구 사이를 직접 전환한 횟수 = **설계상 0회, 운영상 예외 발생 시 로그로 추적**
- **G2. 복붙 횟수**: 사용자가 한쪽 출력을 다른 쪽 입력으로 복붙한 횟수 = **설계상 0회, 운영상 예외 발생 시 로그로 추적** (G1c 와 짝)
...
> **중요**: 본 절과 §6~§7 의 흐름/명령은 **Phase 0a/0b 통과 시 채택될 후보안**이다. 검증 전까지는 인터페이스 계약과 상태 요구사항을 설명하는 설계 초안이지, 확정 구현 명세가 아니다.

### 4-1. 한 줄 요약
**Claude Code 의 슬래시 커맨드 안에서 Codex CLI 를 Bash subprocess 로 호출**하는 방식을 현재 1순위 후보안으로 두고, 결정 게이트만 사용자에게 남긴다.
...
### 4-3. 핵심 메커니즘
- **Codex 호출**: `codex exec --cd $(pwd)` 형태를 **현재 후보 인터페이스**로 가정. Phase 0b 전까지는 추상 명칭 `$CODEX_CMD` 로 읽고, 실제 서브커맨드/플래그는 검증 후 확정
...
> **조건부 실행 명세**: 본 §6 상태머신은 **Phase 0a/0b 통과 시 유효한 in-command 후보안**이다. Phase 0a 가 실패하면 동일 계약을 out-of-command 상태머신(`resume` 커맨드 또는 외부 하네스)으로 재기술해야 하며, 아래 `stage`/`gate`/`run` 개념은 유지하되 실행 매체만 바뀐다.
...
>
> **degraded review 의 위험도 차등 원칙 (v8)**: `/plan` 은 ADR/경계 변경이 없으면 기본 진행 가능, `/work` 는 `diff_lines > threshold`, `security/auth/payment/config` touched, split 리뷰 발생 시 기본값을 "중단/재시도" 로 상향한다. 임계값은 Phase 0 에서 확정한다.
...
2. **lock 획득 + `sync` 상당 로직 실행**: nested slash 호출은 Phase 0a 전까지 가정하지 않고, 기존 `/next`/`/sync` 의 판단 로직을 인라인 또는 공통 스크립트로 재사용
...
5. 계획서 초안 작성 (템플릿은 §10-1 참조). 작업 항목은 stable id (`P1.`, `P2.`, ...) 필수. 성공 후 `stage=plan.draft`
...
7. `stage=plan.review` 기록 후 Codex 리뷰 호출 (§7-1 호출 규약 — 프롬프트에 `${RUN_ID}` 주입). raw JSON 은 `.cache/codex-reviews/plan-<task-id>-<ts>.json` 에 저장
8. 결과 파싱 후 §7-3 스키마 검증. `run_id` 응답값이 주입값과 일치하지 않으면 §7-5-A fallback
9. **조건부 (GP-2)**: `result=ok` 이고 응답 items 에 severity ∈ {P0, P1} 이 1건 이상 → 사용자 표 제시 + 게이트 실행. `result=ok` + P0/P1 0건 → 자동 통과. `result!=ok` 이면 **GP-2b** 실행
10. 선택 항목을 계획서에 반영. state 의 `review_runs[]` 에 `{run_id, command: "plan", loop, ts, raw_path, result, accepted_ids, rejected_ids, deferred_ids}` append, `pending_run` 제거
11. 루프 판정: `attempts_by_command.plan < 3` 이고 사용자가 [4]/[5] 미선택이면 Step 6로 복귀
12. 종료 시 state 파일에 `stage: "plan.done"` 기록

**중단 후 재개**: `stage`, `loop_count_by_command.plan`, `review_runs[]` (`command=="plan"` 인 run 만), `pending_run` 로 재진입 지점 결정
...
3. `stage=work.impl` 기록 후 계획서 항목 순회하며 구현. 항목 완료 시 state 의 `completed_plan_items[]` 에 해당 **stable id** (예: `"P1"`, `"P2"` — §10-1 규약) 를 append
...
6. **run 예약 기록**: `run_id="work:<utc-ts>:<session_id>:<attempt>"` 생성 후 state 에 `pending_run` 으로 **원자 저장**. `loop_count_by_command.work`, `attempts_by_command.work`, `codex_attempts_cycle_total` 은 이 시점에 함께 증가
7. `stage=work.review` 기록 후 Codex diff 리뷰 호출 (§7-2 호출 규약, diff 크기 분기는 §7-4 — 프롬프트에 `${RUN_ID}` 주입)
8. 결과 파싱 후 §7-3 스키마 검증. `run_id` 불일치 시 §7-5-A fallback. 표 제시 — raw JSON 은 `.cache/...` 에 저장
9. **조건부 (GW-2)**: `result=ok` 이고 응답 items 에 P0/P1 ≥ 1건 → 사용자 표 + 게이트. `result=ok` + P0/P1 0건 → 자동 통과. `result!=ok` 이면 **GW-2b** 실행. P2 는 어느 쪽이든 audit log 에만 기록
10. 선택 항목 적용. state 의 `review_runs[]` 에 해당 run 엔트리 (`run_id`, `command: "work"`, `loop`, `ts`, `raw_path`, `result`, `accepted_ids`, `rejected_ids`, `deferred_ids`) append, `pending_run` 제거 → Step 6로 루프 (`attempts_by_command.work < 3` 조건)
11. 종료 시 state 파일에 `stage: "work.done"` 기록

**중단 후 재개**: state 의 `stage`, `loop_count_by_command`, `completed_plan_items` (stable id), `review_runs` (각 run 별 수용/거부), `branch` 로 재진입
...
**전제**: `/work` 완료, working tree 에 변경 존재, state.json 의 `stage` 가 `work.done`, `state.branch` 가 현재 checkout branch 와 일치
**산출물**:
- 작업별 git 커밋 (여러 개)
- 원격 push (사용자 명시 동의 후)
- GitHub PR
...
9. **`done` 상당 로직 수행** (TASKS `🔄`→`✅`, progress, ADR 갱신) — nested slash 호출 미검증 시 기존 `/done` 의 파일 갱신 로직을 공통 스크립트/인라인으로 재사용
...
- `review_runs[]`: **각 Codex 호출을 1개 run 으로 기록**. Codex 출력의 `items[].id` 가 호출마다 1,2,3... 으로 reset 되므로 단일 배열로 합치면 loop 간 충돌 발생. run 별로 분리 저장해 감사/회고 시 "어느 호출의 몇 번 항목을 수용/거부했는지" 를 추적
  - `run_id`: `"{command}:{utc-ts}:{session_id}:{attempt}"` 형식의 복합 키. loop 번호와 별개로 **예약 시점에 유일하게 생성**되어 crash/replay 후 재사용되지 않음
...
- `loop_count_by_command`: 커맨드별 루프 수 (표시/감사용)
- `attempts_by_command`: 커맨드별 실제 Codex 호출 수. `/plan`, `/work` 각각의 상한 3회 판정은 이 값 기준
...
# heredoc 은 quote 없이 (unquoted EOF) — ${TASK_ID}/${RUN_ID} 등 변수 치환 허용
...
[출력 필수] 응답 JSON 의 최상위에 반드시 "run_id": "${RUN_ID}" 를 그대로 포함. run_id 를 변경/생략/요약하지 말 것. items[].id 는 1부터 시작해 본 응답 내에서만 유일.
[스키마] (§7-3 참조)
EOF
```
...
### 7-3. 출력 프로토콜 (강제 JSON 스키마) — v4: run_id 추가 (F2 대응)
```json
{
  "run_id": "work:20260418T161000Z:01HFABC:1",
  "summary": "한 줄 종합 코멘트",
...
| 필드 | 필수 | 값 |
|------|------|------|
| `run_id` | ✓ (v4) | `"{command}:{utc-ts}:{session_id}:{attempt}"`. 호출 전 state 에 예약된 값과 동일해야 함 |
| `summary` | ✓ | 1줄 |
...
|-----------|------|
| ~500줄 | 단일 호출, 전체 리뷰 |
| 500~2000줄 | 사용자에게 "분할 리뷰?" 확인 후 **최대 3개 chunk** 로 분할. 우선순위는 (1) 실행 코드 (2) 테스트 누락 후보 (3) 설정/문서. 나머지는 "미리뷰 범위" 로 audit log 에 남김 |
...
- Q6-1. Phase 0b 전까지 문서상의 `codex exec --cd` 는 예시 표기인데, 실제 확정 인터페이스를 어떤 추상명(`$CODEX_CMD`)으로 감쌀 것인가?
...
| 0c | macOS / `gh` / git 운영 환경 확인 | `timeout`/`gtimeout`, `gh --version`, `gh auth status`, origin remote, push 권한 검증 |
...
| **decision prompt 수** (G1b) | `gate-events.tsv` 의 `shown`, `auto_passed`, `user_choice` 집계 | 베이스라인 측정 후 결정. degraded review 게이트는 별도 집계 |
| **context switch 수** (G1c) | 설계상 0 + 예외 fallback 이벤트 로그 | **0** |
| **복붙 수** (G2) | 설계상 0 + 수동 fallback 이벤트 로그 | **0** |
...
| 자동 통과율 | `gate-events.tsv` 의 `auto_passed=true` 비율 | 정상 경로에서 높을수록 좋음 |
| degraded 승인율 | `result="degraded"` AND `risk_level="high"` 인 행 중 `degraded_accepted=true` 비율 | 위험 task 에서 낮을수록 좋음 |
...
모든 호출 메트릭은 `.cache/codex-reviews/_metrics.tsv`, 게이트 메트릭은 `.cache/codex-reviews/gate-events.tsv` 에 append. `context switch`, `복붙` 은 설계상 0을 기본으로 두되, 예외 fallback 발생 시 이벤트 로그로 남긴다. 설문은 보조 수단이다.
...
- [ ] **0a. 슬래시 커맨드 실행 모델 검증** (최우선)
  ...
  - [ ] 다단계 게이트 동작 가능성 확인
  - [ ] nested slash 불가 시 `/next`/`/sync`/`/done` 로직 재사용 방식 (인라인 vs 공통 스크립트) 확정
  - [ ] 인터럽트/출력 truncation 이 다단계 루프를 깨지 않는지 확인
  - [ ] 결과가 C (불가능) 면 차기 버전 재설계로 전환 — 이하 단계 무효
- [ ] **0b. codex CLI 명세 확정**
  - [ ] §11-1 Q1~Q6 답변 확보
  - [ ] codex CLI 단독 테스트
  - [ ] JSON 출력 강제 5회 호출 검증 (성공률 기록)
- [ ] **0c. macOS / `gh` / git 환경 확인**
  - [ ] `timeout`/`gtimeout` 존재 여부
  - [ ] §2-2 fallback 패턴 검증
  - [ ] `gh --version`, `gh auth status`, origin remote, push 권한 확인
- [ ] `.gitignore` 에 `docs/plans/*.state.json`, `docs/plans/*.lock/`, `.cache/` 추가 (lock 은 디렉토리, §6-4-4)
- [ ] state atomic write 규약 (`tmp` + `mv`) 검증
